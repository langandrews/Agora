''' MAINCODE.py
    ...
    This is the primary driver for the Stock Market Game. 
    Generates the stock graphs based on prices.py stock class, 
    feeds and receives various monetary values, and checks for 
    wins/losses. Hopefully more to come!
    
    Game instructions are in the GUI.
...
class GraphGui:
    def __init__(self,window):
        ...
        self.canvas.create_text(300,75,text='Hello, and welcome to Q\'s STOCK MARKET GAME!',tags='starttext', fill='#a90000',font=('impact',16))
...
option to re-start the game with 10 Thousand dollars in Liquid Assets.',tags='infotext',width=550)
    ...
    def checkend(self):
        if self.nworth.get_networth() <= 100 and self.nworth.get_invested() == 0:
            ...
            self.sell_button.config(state = DISABLED)
            self.skip_button.config(state = DISABLED)
            self.Instruct_button.config(state = DISABLED)
            self.canvas_clean()
            loser_funnies = ['Here, take a McDonald\'s application.','Repeat after me: "Would you like fries with that?"','Oh, by the way, that was real money. We borrowed it from the Russian Mob, and we did it under your name.',
                             'This is what happens when you major in the Liberal Arts.','You might want to wait on sending Warren Buffet an application...','We don\'t score stocks like golf.',
    ...
    def usr_sell(self,offset=1):
        ...
        self.nworth.set_networth(round(oldnetworth+newnetworth,2))
