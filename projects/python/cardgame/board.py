...
class Board:
    ...
    def findCard(self, cardNum, cardSuit):
        ...
        try:
            ...
            print("Could not create the card with -->" + cardNum + "<->" + cardSuit + "<--")
            raise e
        for row in range(self.NUM_ROWS):
            for col in range(self.NUM_COLS):
    ...
    def findPlayableCards(self):
        ...
        # then find all 2s (not in column 0) and add them to the list
        # to be returned.
        roomForATwo = False
        for row in range(self.NUM_ROWS):
        ...
        if roomForATwo:
            for row in range(self.NUM_ROWS):
                # skip looking for 2s in the 1st column.
    ...
    def getCardsInPlace(self):
        '''Return a list of the (x, y) offsets for the cards that have been
