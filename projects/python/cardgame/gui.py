...
class App:
    ...
    def __init__(self, window):
        ...
        btNewGame = Button(self._buttonFr, text="New Game", command = self.newGame)
        ...
        cardsInPlaceNum = Label(self._buttonFr, textvariable=self._cardsInPlaceText)
        ...
        self._highScoreLbl = Label(self._buttonFr, text="High Score: " + str(self._highScore))
    ...
    def removeAces(self):
        ...
        for card in self._removedAces:
            self._canv.itemconfig(self._card2ImgDict[id(card)].getTag(), state=HIDDEN)
    ...
    def initNewGame(self):
        ...
        self._scoreText.set("Score: %d (10 pts per card this round)" % self._score)
    ...
    def onCardClick(self, event):
        ...
        res = self._board.canCardBeMoved(card)
        if res is None:
    ...
    def isEndOfRoundOrGame(self):
        ...
        if self._board.gameCompletelyDone():
            ...
            if newhigh:
                ...
                self._highScoreLbl.config(text="High Score: " + str(self._highScore))
        ...
        if not self._board.moreMoves():
            self.displayMesg("No more moves. Click Next Round to continue.", 2000)
